...
GITHUB_API_URL = "https://api.github.com/repos/ADEL-MAHMOUD10/Database/contents/progress.db"
...
def download_db_from_github():
    ...
    if response.status_code == 200:
        db_path = os.path.join("uploads", "progress.db")
...
def create_db():
    conn = sqlite3.connect(os.path.join(app.config['UPLOAD_FOLDER'], 'progress.db'))
    c = conn.cursor()
...
def upload_audio_to_assemblyai(audio_path):
    ...
    while True:
        ...
        if transcription_result['status'] == 'completed':
            ...
            progress["status"] = 100
            update_progress(transcript_id, 100, "completed")
    ...
    try:
        if request.method == 'POST':
            ...
            if file_extension in [".mp4", ".wmv", ".mov", ".mkv", ".h.264"]:
                ...
                os.remove(file_path)
                update_progress(transcript_id=None, status=0, message="Error File")
                return render_template("error.html")
...
def download_subtitle(transcript_id):
    if request.method == 'POST':
        ...
        if response.status_code == 200:
            ...
            subtitle_file = f"subtitle_{time_file}.{file_format}"
...
if __name__ == '__main__':
    ...
    if not os.path.exists("uploads/progress.db"):
        download_db_from_github()  
    create_db()  
